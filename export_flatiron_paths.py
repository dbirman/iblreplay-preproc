...
def export_paths(eid):
    ...
    for pi, probe in enumerate(probes):
        ...
        if len(dataset)>0:
            ...
            # also load the UUIDs and save the indexes that correspond to our clusters
            cluster = one.load_object(eid, obj='clusters', attribute=['uuids'], collection=f'alf/{probe}')
            uuids = cluster['uuids']['uuids'] # why is it two deep?
            
            uuidfile = save_path.joinpath(f'uuid_idx_{eid}_{probe}.txt')
...
# get all session IDs
sess_id = np.load("data/brainwide_map_eids_ephys_histology.npy")
